...
bool computeScore(float4 oldFly, inout float4 newFly)
{
	float oldScore = tex2D(bordersMapSampler, oldFly.rg).r;
	float newScore = tex2D(bordersMapSampler, newFly.rg).r;
...
{
	float4 oldFly = tex2D(previousPopulationMapSampler, input.TexCoord);

	float4 noiseData = tex2D(noiseMapSampler, input.TexCoord + oldFly.xy + float2(randomNoiseOffset, randomNoiseOffset));

	float commonMultiply = 2;

	float2 noiseValue = (noiseData.xy - float2(0.5, 0.5));

	if(oldFly.b <= 0)
		commonMultiply *= 6 * noiseValue.x;

	float2 noisePixelDirection = noiseValue * 2 * commonMultiply * halfPixel; //((noiseData.xy - float2(0.5, 0.5)) * 2) * 2 * halfPixel;
...
VSO VS_PLOT(VSI_PLOT input)
{
	...
	output.Position = float4(2 * (float3(fly.r, 1 - fly.g, 0) - float3(0.5,0.5,0)), 1) + float4(2 * halfPixel * input.Position.z,0,0);
